...
collection =db.mycollection
users_collection_3=db.recruiter
...
def upload_file():
    ...
    if file:
        # Read the PDF file content as a file-like object
        ...
        data['pdf_id'] = str(pdf_id)  # Store the GridFS file ID
...
def download_file():
    # app.config['FILES_FOLDER'] = 'cloud'
    # file_path = app.config['FILES_FOLDER'] + '/FORMAT_RESUME.pdf'
    # print(file_path)
    return send_file('uploads/FORMAT_RESUME.pdf', as_attachment=True)
# @app.route('/joel_data')
# def joel_data():
